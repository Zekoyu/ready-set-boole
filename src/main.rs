...
fn main() {
    ...
    for i in 0..100 {
      for j in 0..100 {
        let encoded = map_reverse_map::map(i, j);
        let decoded = map_reverse_map::reverse_map(encoded);
        // println!("i: {i}, j: {j}, encoded: {encoded}, decoded: {}; {}", decoded.0, decoded.1);
        if decoded.0 != i || decoded.1 != j {
          println!("Map / unmap is not working with {i} and {j}");
          working = false;
          break 'outer;
        }
      }
    ...
    if working {
      println!("map / reverse_map working");
    }
